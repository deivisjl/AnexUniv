...
namespace Service
{
    public interface IStudentService
    {
        ...
        ResponseHelper Review(ReviewPerCourse model);
    ...
    public class StudentService : IStudentService
    {
        ...
        private readonly IRepository<ReviewPerCourse> _reviewPerCourseRepo;
        private readonly IRepository<LessonsPerCourse> _lessonPerCourseRepo;
        private readonly IRepository<CourseLessonsLearnedPerStudent> _lessonLearnedRepo;

        public StudentService(
            IDbContextScopeFactory dbContextScopeFactory,
            IRepository<Course> courseRepo,
            IRepository<UsersPerCourses> userPerCourseRepo,
            IRepository<ReviewPerCourse> reviewPerCourseRepo,
            IRepository<LessonsPerCourse> lessonPerCourseRepo,
            IRepository<CourseLessonsLearnedPerStudent> lessonLearnedRepo
        )
        {
        ...
        public IEnumerable<CoursePerStudentListView> GetAll(string userId)
        {
            ...
            try
            {
                using (var ctx = _dbContextScopeFactory.CreateReadOnly())
                {
                    ...
                    var progressPerUser = ctx.GetEntity<CourseLessonsLearnedPerStudent>();
        ...
        public ResponseHelper Review(ReviewPerCourse model)
        {
        ...
        public ResponseHelper MarkAsLearned(int courseId, int lessonId, string userId)
        {
            ...
            try
            {
                using (var ctx = _dbContextScopeFactory.Create())
                {
                    ...
                    {
                        _lessonLearnedRepo.Insert(new CourseLessonsLearnedPerStudent
                        {
                            UserId = userId,
                            LessonId = lessonId
                        });
